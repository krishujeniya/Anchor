...
3. Actively search the codebase for undocumented dependencies, overlapping abstractions, or potential race conditions that the implementation plan might hit.
4. **Output Constraint**: You MUST distill your findings into a single summary of NO MORE than 1,000 tokens. 
5. Do NOT suggest solutions. Your job is purely to identify the blast radius and hidden dangers. The orchestrator will use your output to write the risk note for the `ANALYZE` gate.
